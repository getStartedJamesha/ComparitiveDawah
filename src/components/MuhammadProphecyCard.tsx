import type { MuhammadProphecy } from "../data/types";

export default function MuhammadProphecyCard({ item }: { item: MuhammadProphecy }) {
  const unfulfilled = item.category === "unfulfilled";
    ...
    >
      ...
      <div className="mt-4 rounded-xl bg-slate-50 p-4 dark:bg-slate-800/60">
        <blockquote className="font-serif text-base leading-relaxed text-slate-800 dark:text-slate-100">
          &ldquo;{item.source.text}&rdquo;
        </blockquote>
        <p className="mt-3 text-sm font-medium text-slate-700 dark:text-slate-200">
          {item.source.reference}
        </p>
        {item.source.grade && (
          <p className="text-xs text-slate-400">Grading: {item.source.grade}</p>
        )}
        <a
      ...
      {unfulfilled && (
        <div className="mt-4 rounded-xl border border-dashed border-amber-300 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/30">
          <p className="text-sm font-medium text-amber-800 dark:text-amber-400">
            Not yet fulfilled — this describes an awaited future event, not a
            past one that can currently be checked against history.
          </p>
        </div>
      )}
